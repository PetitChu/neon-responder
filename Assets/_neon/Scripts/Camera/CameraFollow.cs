...
namespace BrainlessLabs.Neon
{
	...
	public class CameraFollow : MonoBehaviour
	{
		...
		[Header("Player Targets")] [SerializeField]
		private GameObject[] targets; //target(s) to follow
		...
		private float yOffset = 0; // the height offset of the camera relative to it's target

		[HideInInspector] public float additionalYOffset = 0; //used by cam shake

		[Header("Damp Settings")] [SerializeField]
		private float DampX = 3f;

		[SerializeField] private float DampY = 1f;

		[Header("View Area")] [SerializeField] private float Left;
		[SerializeField] private float Right;
		[SerializeField] private float Top;
		[SerializeField] private float Bottom;
		[SerializeField] private bool showViewAreaRect; //draws the borders of the camera view and view area

		[Header("BackTracking")] [SerializeField]
		private bool allowBacktracking; //enable or disable moving backwards in the level

		[SerializeField] private float BackTrackMargin; //a small margin to allow moving back slightly

		[Header("Level Bound")] public LevelBound levelBound;
		...
		void Update()
		{
			...
			transform.position = new Vector3(Mathf.Clamp(camX, Left, Right), Mathf.Clamp(camY, Bottom, Top),
			...
			if (levelBound != null)
			{
				...
				if (cameraRightEdge > levelBoundPos)
					Camera.main.transform.position = new Vector3(levelBoundPos - halfSize,
			...
			if (!allowBacktracking)
			{
				...
				if (cameraLeftEdge - BackTrackMargin > Left)
					Left = cameraLeftEdge - BackTrackMargin; //move left border forward
			}
		...
		void ShowCameraView()
		{
			...
			Vector2 topLeft = new Vector2(transform.position.x - cameraHalfWidth,
				transform.position.y + cameraHalfHeight);
			Vector2 bottomLeft = new Vector2(transform.position.x - cameraHalfWidth,
				transform.position.y - cameraHalfHeight);
			Vector2 topRight = new Vector2(transform.position.x + cameraHalfWidth,
				transform.position.y + cameraHalfHeight);
			Vector2 bottomRight = new Vector2(transform.position.x + cameraHalfWidth,
				transform.position.y - cameraHalfHeight);
