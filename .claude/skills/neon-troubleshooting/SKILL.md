...
- **Fix:** One-line set in `SpawnUnit` after injection, for enemy units — grab the component and set the field directly: `var eb = instance.GetComponent<EnemyBehaviour>(); if (eb != null) eb.AI_Active = true;`. Note `AI_Active` is a **public field**, not a method — there is no `SetActive` on `EnemyBehaviour`. Scope it to enemies (players don't have `EnemyBehaviour`). Reproduce first: spawn a wave, confirm `targetSpotted` true + no attacks, apply, confirm attacks.
